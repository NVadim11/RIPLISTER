    ...
    (() => {
        ...
        function profileTabs() {
            ...
            if (tabs.length > 0) {
                ...
                tabs.forEach(((tabsBlock, index) => {
                    tabsBlock.classList.add("_tab-init");
                    tabsBlock.setAttribute("profile-tabs-index", index);
                    tabsBlock.addEventListener("click", setTabsAction);
                    initTabs(tabsBlock);
                }));
                let mdQueriesArray = dataMediaQueries(tabs, "tabs");
            ...
            function setTitlePosition(tabsMediaArray, matchMedia) {
                tabsMediaArray.forEach((tabsMediaItem => {
                    tabsMediaItem = tabsMediaItem.item;
                    let tabsTitles = tabsMediaItem.querySelector("[profile-tabs-titles]");
                    let tabsTitleItems = tabsMediaItem.querySelectorAll("[profile-tabs-title]");
                    let tabsContent = tabsMediaItem.querySelector("[profile-tabs-body]");
                    let tabsContentItems = tabsMediaItem.querySelectorAll("[profile-tabs-item]");
                    tabsTitleItems = Array.from(tabsTitleItems).filter((item => item.closest("[profile-tabs]") === tabsMediaItem));
                    tabsContentItems = Array.from(tabsContentItems).filter((item => item.closest("[profile-tabs]") === tabsMediaItem));
                    tabsContentItems.forEach(((tabsContentItem, index) => {
                        if (matchMedia.matches) {
                            tabsContent.append(tabsTitleItems[index]);
                            tabsContent.append(tabsContentItem);
            ...
            function initTabs(tabsBlock) {
                let tabsTitles = tabsBlock.querySelectorAll("[profile-tabs-titles]>*");
                let tabsContent = tabsBlock.querySelectorAll("[profile-tabs-body]>*");
                const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
                const tabsActiveHashBlock = tabsActiveHash[0] == tabsBlockIndex;
                if (tabsContent.length) tabsContent.forEach(((tabsContentItem, index) => {
                    tabsTitles[index].setAttribute("profile-tabs-title", "");
                    tabsContentItem.setAttribute("profile-tabs-item", "");
                    if (tabsActiveHashBlock && index == tabsActiveHash[1]) tabsTitles[index].classList.add("_profile-active");
                    tabsContentItem.hidden = !tabsTitles[index].classList.contains("_profile-active");
                }));
            }
            function setTabsStatus(tabsBlock) {
                let tabsTitles = tabsBlock.querySelectorAll("[profile-tabs-title]");
                let tabsContent = tabsBlock.querySelectorAll("[profile-tabs-item]");
                const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
                function isTabsAnamate(tabsBlock) {
                    if (tabsBlock.hasAttribute("profile-tabs-animate")) return tabsBlock.dataset.tabsAnimate > 0 ? Number(tabsBlock.dataset.tabsAnimate) : 500;
                }
                const tabsBlockAnimate = isTabsAnamate(tabsBlock);
                if (tabsContent.length > 0) {
                    const isHash = tabsBlock.hasAttribute("profile-tabs-hash");
                    tabsContent = Array.from(tabsContent).filter((item => item.closest("[profile-tabs]") === tabsBlock));
                    tabsTitles = Array.from(tabsTitles).filter((item => item.closest("[profile-tabs]") === tabsBlock));
                    tabsContent.forEach(((tabsContentItem, index) => {
            ...
            function setTabsAction(e) {
                const el = e.target;
                if (el.closest("[profile-tabs-title]")) {
                    const tabTitle = el.closest("[profile-tabs-title]");
                    const tabsBlock = tabTitle.closest("[profile-tabs]");
                    if (!tabTitle.classList.contains("_profile-active") && !tabsBlock.querySelector("._slide")) {
                        let tabActiveTitle = tabsBlock.querySelectorAll("[profile-tabs-title]._profile-active");
                        tabActiveTitle.length ? tabActiveTitle = Array.from(tabActiveTitle).filter((item => item.closest("[profile-tabs]") === tabsBlock)) : null;
                        tabActiveTitle.length ? tabActiveTitle[0].classList.remove("_profile-active") : null;
                        tabTitle.classList.add("_profile-active");
                        setTabsStatus(tabsBlock);
                    }
                    e.preventDefault();
                }
        ...
        function bioTabs() {
            ...
            function initTabs(tabsBlock) {
                ...
                if (tabsContent.length) tabsContent.forEach(((tabsContentItem, index) => {
                    ...
                    if (tabsActiveHashBlock && index == tabsActiveHash[1]) tabsTitles[index].classList.add("_bio-active");
            ...
            function setTabsStatus(tabsBlock) {
                ...
                const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
                function isTabsAnamate(tabsBlock) {
                    if (tabsBlock.hasAttribute("bio-tabs-animate")) return tabsBlock.dataset.tabsAnimate > 0 ? Number(tabsBlock.dataset.tabsAnimate) : 500;
                }
                const tabsBlockAnimate = isTabsAnamate(tabsBlock);
                if (tabsContent.length > 0) {
        ...
        if (showLogPass) showLogPass.addEventListener("click", (function(e) {
        ...
        class contactThanks {
            constructor(options) {
                let config = {
                    logging: true,
                    init: true,
                    attributeOpenButton: "data-contactThanks-popup",
                    attributeCloseButton: "data-contactThanks-close",
                    fixElementSelector: "[data-lp]",
                    classes: {
                        popup: "contactThanks",
                        popupWrapper: "contactThanks__wrapper",
                        popupContent: "contactThanks__content",
                        popupActive: "contactThanks_show",
                        bodyActive: "contactThanks-show"
                    },
                    focusCatch: true,
                    closeEsc: true,
                    bodyLock: true,
        ...
        class contactUsModal {
            constructor(options) {
                let config = {
                    logging: true,
                    init: true,
                    attributeOpenButton: "data-contactUsModal-open",
                    attributeCloseButton: "data-contactUsModal-close",
                    fixElementSelector: "[data-lp]",
                    classes: {
                        popup: "contactUsModal",
                        popupWrapper: "contactUsModal__wrapper",
                        popupContent: "contactUsModal__content",
                        popupActive: "contactUsModal_show",
                        bodyActive: "contactUsModal-show"
                    },
                    focusCatch: true,
                    closeEsc: true,
                    bodyLock: true,
        ...
        flsModules.contactUsModal = new contactUsModal({});
        ...
        function initSliders() {
            ...
            if (document.querySelector(".memoInfoSlider")) new swiper_core_Swiper(".memoInfoSlider", {
                modules: [ Pagination ],
                grabCursor: true,
                effect: "slide",
                speed: 800,
        ...
        document.querySelector(".personalsEdit__newPassErr");
        document.querySelector(".personalsEdit__repeatPassErr");
        ...
        function profile_validateForm(formSelector, callback) {
            const formElement = document.querySelector(formSelector);
            const validationOptions = [ {
                attribute: "minlength",
                isValid: input => input.value && input.value.length >= parseInt(input.minLength, 10)
        ...
        function savePersonalData(formElement) {
            ...
            if (document.getElementById("personalsEdit__newPass").value !== document.getElementById("personalsEdit__repeatPass").value) {
                ...
                return false;
            } else document.querySelector(".personalsEdit__repeatPassErr").style.display = "none";
            console.log(formObject);
        }
        if (personalsEditForm) profile_validateForm("#personalsEdit__form", savePersonalData);
        let up_to_top_requestAnimationFrame = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame;
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            window.addEventListener("scroll", (function() {
                if (!isRequestingAnimationFrame) {
                    up_to_top_requestAnimationFrame(filterGoTopButtonVisibility);

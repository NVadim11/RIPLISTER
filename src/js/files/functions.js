...
export function spollers() {
	...
	if (spollersArray.length > 0) {
		...
		function setSpollerAction(e) {
			...
			if (el.closest('summary') && el.closest('[data-spollers]')) {
				...
				if (el.closest('[data-spollers]').classList.contains('_spoller-init')) {
					...
					if (spollerTitle.classList.contains('_spoller-active')) {	
						document.querySelectorAll('.spollerClose').forEach(spollerCloseBtn => {
			...
			if (el.closest('.instructionsItem__title-close')) {
...
export function profileTabs() {
	...
	function initTabs(tabsBlock) {
		...
		// if (tabsActiveHashBlock) {
		// 	const tabsActiveTitle = tabsBlock.querySelector('[profile-tabs-titles]>._profile-active');
		// 	tabsActiveTitle ? tabsActiveTitle.classList.remove('_profile-active') : null;
		// }
		if (tabsContent.length) {
			//tabsContent = Array.from(tabsContent).filter(item => item.closest('[data-tabs]') === tabsBlock);
			//tabsTitles = Array.from(tabsTitles).filter(item => item.closest('[data-tabs]') === tabsBlock);
			tabsContent.forEach((tabsContentItem, index) => {
				...
				if (tabsActiveHashBlock && index == tabsActiveHash[1]) {
					tabsTitles[index].classList.add('_profile-active');
				}
	...
	function setTabsStatus(tabsBlock) {
		...
		const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
		function isTabsAnamate(tabsBlock) {
			if (tabsBlock.hasAttribute('profile-tabs-animate')) {
				return tabsBlock.dataset.tabsAnimate > 0 ? Number(tabsBlock.dataset.tabsAnimate) : 500;
			}
		}
		const tabsBlockAnimate = isTabsAnamate(tabsBlock);
...
export function billTabs() {
	...
	function initTabs(tabsBlock) {
		...
		if (tabsContent.length) {
			...
			tabsContent.forEach((tabsContentItem, index) => {
				...
				if (tabsActiveHashBlock && index == tabsActiveHash[1]) {
					tabsTitles[index].classList.add('_bill-active');
				}
	...
	function setTabsStatus(tabsBlock) {
		...
		const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
		function isTabsAnamate(tabsBlock) {
			if (tabsBlock.hasAttribute('bill-tabs-animate')) {
				return tabsBlock.dataset.tabsAnimate > 0 ? Number(tabsBlock.dataset.tabsAnimate) : 500;
			}
		}
		const tabsBlockAnimate = isTabsAnamate(tabsBlock);
...
export function menuInit() {
	if (document.querySelector(".scroll-lock")) {
		document.addEventListener("click", function (e) {
			if (bodyLockStatus && e.target.closest('.scroll-lock')) {
				bodyLockToggle();
...
export function addBackground() {
	window.addEventListener("load", function (e) {
		const showMoreBlocks = document.querySelectorAll('[data-bgColor]');
		let showMoreBlocksRegular;
		let mdQueriesArray;
		if (showMoreBlocks.length) {
			// Отримання звичайних об'єктів
		...
		function initItem(showMoreBlock, matchMedia = false) {
			showMoreBlock = matchMedia ? showMoreBlock.item : showMoreBlock;
			let showMoreContent = showMoreBlock.querySelectorAll('[data-bgColor-content]');
			let showMoreButton = showMoreBlock.querySelectorAll('[data-bgColor-button]');
			showMoreContent = Array.from(showMoreContent).filter(item => item.closest('[data-bgColor]') === showMoreBlock)[0];
			showMoreButton = Array.from(showMoreButton).filter(item => item.closest('[data-bgColor]') === showMoreBlock)[0];
			const hiddenHeight = getHeight(showMoreBlock, showMoreContent);
			if (matchMedia.matches || !matchMedia) {
				if (hiddenHeight < getOriginalHeight(showMoreContent)) {
					_slideUp(showMoreContent, 0, showMoreBlock.classList.contains('bgColor-active') ? getOriginalHeight(showMoreContent) : hiddenHeight);
					showMoreButton.hidden = false;
				} else {
					_slideDown(showMoreContent, 0, hiddenHeight);
					showMoreButton.hidden = true;
		...
		function getHeight(showMoreBlock, showMoreContent) {
			...
			if (showMoreType === 'items') {
				...
				if (viewportWidth >= 1919.98) {
					showMoreTypeValue = showMoreContent.dataset.showmoreContent ? showMoreContent.dataset.showmoreContent : 0;
				} else if (viewportWidth >= 1439.98) {
					showMoreTypeValue = showMoreContent.dataset.showmoreContent ? showMoreContent.dataset.showmoreContent : 0;
				} else if (viewportWidth >= 1279.98) {
					showMoreTypeValue = showMoreContent.dataset.showmoreContent ? showMoreContent.dataset.showmoreContent : 0;
		...
		function showMoreActions(e) {
			...
			const targetType = e.type;
			if (targetType === 'click') {
				if (targetEvent.closest('[data-bgColor-button]')) {
					const showMoreButton = targetEvent.closest('[data-bgColor-button]');
					const showMoreBlock = showMoreButton.closest('[data-bgColor]');
					const showMoreContent = showMoreBlock.querySelector('[data-bgColor-content]');
					const showMoreSpeed = showMoreBlock.dataset.showmoreButton ? showMoreBlock.dataset.showmoreButton : '500';
					const hiddenHeight = getHeight(showMoreBlock, showMoreContent);
					if (!showMoreContent.classList.contains('_slide')) {
						showMoreBlock.classList.contains('bgColor-active') ? _slideUp(showMoreContent, showMoreSpeed, hiddenHeight) : _slideDown(showMoreContent, showMoreSpeed, hiddenHeight);
						showMoreBlock.classList.toggle('bgColor-active');
					}
				}
			} else if (targetType === 'resize') {
				showMoreBlocksRegular && showMoreBlocksRegular.length ? initItems(showMoreBlocksRegular) : null;

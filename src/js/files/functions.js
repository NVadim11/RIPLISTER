...
export function profileTabs() {
	...
	function setTitlePosition(tabsMediaArray, matchMedia) {
		tabsMediaArray.forEach((tabsMediaItem) => {
			...
			tabsTitleItems = Array.from(tabsTitleItems).filter(
				(item) => item.closest("[profile-tabs]") === tabsMediaItem
			);
			tabsContentItems = Array.from(tabsContentItems).filter(
				(item) => item.closest("[profile-tabs]") === tabsMediaItem
			);
	...
	function setTabsStatus(tabsBlock) {
		...
		if (tabsContent.length > 0) {
			...
			tabsContent = Array.from(tabsContent).filter(
				(item) => item.closest("[profile-tabs]") === tabsBlock
			);
			tabsTitles = Array.from(tabsTitles).filter(
				(item) => item.closest("[profile-tabs]") === tabsBlock
			);
	...
	function setTabsAction(e) {
		...
		if (el.closest("[profile-tabs-title]")) {
			...
			if (
				!tabTitle.classList.contains("_profile-active") &&
				!tabsBlock.querySelector("._slide")
			) {
				let tabActiveTitle = tabsBlock.querySelectorAll(
					"[profile-tabs-title]._profile-active"
				);
				tabActiveTitle.length
					? (tabActiveTitle = Array.from(tabActiveTitle).filter(
							(item) => item.closest("[profile-tabs]") === tabsBlock
					...
					: null;
				tabActiveTitle.length
					? tabActiveTitle[0].classList.remove("_profile-active")
					: null;
...
export function billTabs() {
	...
	function setTitlePosition(tabsMediaArray, matchMedia) {
		tabsMediaArray.forEach((tabsMediaItem) => {
			...
			tabsTitleItems = Array.from(tabsTitleItems).filter(
				(item) => item.closest("[bill-tabs]") === tabsMediaItem
			);
			tabsContentItems = Array.from(tabsContentItems).filter(
				(item) => item.closest("[bill-tabs]") === tabsMediaItem
			);
	...
	function setTabsStatus(tabsBlock) {
		...
		if (tabsContent.length > 0) {
			...
			tabsContent = Array.from(tabsContent).filter(
				(item) => item.closest("[bill-tabs]") === tabsBlock
			);
			tabsTitles = Array.from(tabsTitles).filter(
				(item) => item.closest("[bill-tabs]") === tabsBlock
			);
	...
	function setTabsAction(e) {
		...
		if (el.closest("[bill-tabs-title]")) {
			...
			if (
				!tabTitle.classList.contains("_bill-active") &&
				!tabsBlock.querySelector("._slide")
			) {
...
export function bioTabs() {
	...
	function setTitlePosition(tabsMediaArray, matchMedia) {
		tabsMediaArray.forEach((tabsMediaItem) => {
			...
			tabsTitleItems = Array.from(tabsTitleItems).filter(
				(item) => item.closest("[bio-tabs]") === tabsMediaItem
			);
			tabsContentItems = Array.from(tabsContentItems).filter(
				(item) => item.closest("[bio-tabs]") === tabsMediaItem
			);
	...
	function setTabsStatus(tabsBlock) {
		...
		if (tabsContent.length > 0) {
			...
			tabsContent = Array.from(tabsContent).filter(
				(item) => item.closest("[bio-tabs]") === tabsBlock
			);
			tabsTitles = Array.from(tabsTitles).filter(
				(item) => item.closest("[bio-tabs]") === tabsBlock
			);
	...
	function setTabsAction(e) {
		...
		if (el.closest("[bio-tabs-title]")) {
			...
			if (
				!tabTitle.classList.contains("_bio-active") &&
				!tabsBlock.querySelector("._slide")
			) {
...
export function dropdownInit() {
	document.addEventListener("click", (e) => {
		...
		if (!isDropdownButton && e.target.querySelector("[data-dropdown]") !== null) return;

		let currentDropdown;
		if (isDropdownButton) {
			currentDropdown = e.target.querySelector("[data-dropdown]");
			currentDropdown.parentElement.classList.toggle("active");
		...
		function getHeight(showMoreBlock, showMoreContent) {
			...
			if (showMoreType === "items") {
				...
				if (viewportWidth >= 1919.98) {
					showMoreTypeValue = showMoreContent.dataset.showmoreContent
						? showMoreContent.dataset.showmoreContent
						: 450;
...
export function addBackground() {
	window.addEventListener("load", function (e) {
		...
		function initItem(showMoreBlock, matchMedia = false) {
			...
			showMoreContent = Array.from(showMoreContent).filter(
				(item) => item.closest("[data-bgColor]") === showMoreBlock
			)[0];
			showMoreButton = Array.from(showMoreButton).filter(
				(item) => item.closest("[data-bgColor]") === showMoreBlock
			)[0];
			...
			if (matchMedia.matches || !matchMedia) {
				if (hiddenHeight < getOriginalHeight(showMoreContent)) {
					_slideUp(
						showMoreContent,
						0,
						showMoreBlock.classList.contains("bgColor-active")
							? getOriginalHeight(showMoreContent)
							: hiddenHeight
					);
					showMoreButton.hidden = false;

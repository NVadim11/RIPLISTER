...
class SelectConstructor {
	...
	setSelectTitleValue(selectItem, originalSelect) {
		...
		this.setSelectIcons(selectItem, originalSelect);
		originalSelect.hasAttribute("data-search") ? this.searchActions(selectItem) : null;
	}

	setSelectIcons(selectItem, originalSelect) {
		const iconLeftDataSet = originalSelect.dataset.iconLeft;
		const iconRightDataSet = originalSelect.dataset.iconRight;

		const iconLeft = originalSelect.hasAttribute("data-icon-left")
			? `<i class="${iconLeftDataSet}"></i>`
			: "";
		const iconRight = originalSelect.hasAttribute("data-icon-right")
			? `<i class="${iconRightDataSet}"></i>`
			: "";
		...
		if (iconLeft || iconRight) {
			...
			selectItemTitle.classList.add("has--icon");

			selectItemTitle
				.querySelector(".select__value")
				.insertAdjacentHTML("afterbegin", `<span class="select__icon">${iconLeft}</span>`);
			selectItemTitle
				.querySelector(".select__value")
				.insertAdjacentHTML("beforeend", `<span class="select__icon">${iconRight}</span>`);
		}
	}
	...
	getOption(selectOption, originalSelect) {
		...
		// место где можно поменять на ссылку
